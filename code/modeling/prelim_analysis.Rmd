...
#effectiveSize(jags.burn_lin_he)
```
...
Comparing the outputs of model 1 (simple Bayesian linear regression) and model 2 (Bayesian linear regression with heteroskedasticity in income data).

Just from WAIC and DIC scores, model 2 appears to perform better as a descriptor of rooftop PV adoption. It has a much lower DIC than model 1, but the WAIC scores were the same. Since both models had the same number of parameters, the difference can't be attributed to that penalty. However, since both models arrived at very similar estimates for the parameter values and their 95% CIs, I question how much the covariates actually contribute to rooftop PV adoption. This is curious, especially considering that in theory, model 2 should be able to model latent, "true" incomes as opposed to the highly error-prone reported income values, and one would expect higher incomes to be correlated with greater adoption because of the upfront and maintenance costs of rooftop PV systems. 

With the exception of x4, or %voBiden, the other covariates seem to have mild or very little effect on y. betas[4], or the slope parameter on x4, suggests that a greater fraction of votes for Biden correlate with an increase in rooftop PV adoption.  

Perhaps x4 is thus the only significant covariate. 
...
res_df <- data.frame(params, model1_vals, model2_vals,
                     model1_ci_l, model1_ci_u, model2_ci_l, model2_ci_u)

fit    <- c("DIC", "WAIC")
model1 <- c(dic.lin_mv, waic.lin_mv$WAIC)
model2 <- c(dic.lin_he, waic.lin_he$WAIC)

fit_df <- data.frame(fit, model1, model2)

knitr::kable(res_df, caption="Table 1: Parameter values for model 1 (Bayesian linear regression) and model 2 (Bayesian linear regression with heteroskedasticity in income data)")

knitr::kable(fit_df, caption="Table 2: DIC and WAIC values for model 1 Bayesian linear regression) and model 2 (Bayesian linear regression with heteroskedasticity in income data)")
```
...
3. model 3: linear regression with just x4, %voBiden
