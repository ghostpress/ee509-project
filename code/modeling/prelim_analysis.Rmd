...
```{r}
c1.pois <- quantile(out_pois_split[,"beta[1]"], c(0.025, 0.975))  ## beta[1] (intercept)
c2.pois <- quantile(out_pois_split[,"beta[2]"], c(0.025, 0.975))  ## beta[2] (slope on x3, %Biden)
```
...
model3_ci_l <- c(cS.lin_err[1], c1.lin_err[1], c2.lin_err[1])
model3_ci_u <- c(cS.lin_err[2], c1.lin_err[2], c2.lin_err[2])
...
fit_model3 <- c(dic.lin_err, waic.lin_err$WAIC)

fit_df <- data.frame(crit, fit_model1, fit_model2, fit_model3)

knitr::kable(res_df, caption="Parameter values for models 1, 2, and 3")

knitr::kable(fit_df, caption="DIC and WAIC values for models 1, 2, and 3")
```
...
MCMCplot(object=jags.burn_lin_mv,
         ...
         ISB=FALSE,
         exact=TRUE,
         main="Comparison of parameters for models 1 and 2")
```
