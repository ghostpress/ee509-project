...
Supplementary: Bayesian multivariate linear regression shows that none of the covariates except x3 have an effect on Y, which is expected given the pairs plot. 
...
for(i in 1:niter) {
  Ey <- out_burn_lin_x3[i, "beta[1]"] + out_burn_lin_x3[i, "beta[2]"]*xpred[i]
  ycred[i,] <- Ey
  ypred[i,] <- rnorm(1, Ey, out_burn_lin_x3[i, "S"])
}
...
# Model S (supplementary): Bayesian multivariate linear regression
...
# Model Sa: Bayesian multivariate linear regression with errors in income data
